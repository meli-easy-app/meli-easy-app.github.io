// ==UserScript==
// @name         Meli Easy — QR Auto Fetch
// @namespace    https://meli-easy-app.github.io/
// @version      2.0
// @description  Busca QR codes automaticamente no Meli Pass quando iniciado pelo app Meli Easy
// @author       Meli Easy
// @match        https://controleacessomlb.providersml.com/*
// @grant        none
...
(async function () {
  ...
  // Só executa se a aba foi aberta pelo app Meli Easy (window.name tem os dados)
  ...
  try {
    ...
    console.info('[MeliEasy] Auto-iniciando —', _inputData.length, 'colaboradores');
  } catch (e) {
    console.error('[MeliEasy] Erro ao ler dados do window.name:', e);
    return;
  }
  ...
  // Botão cancelar no banner
  _bkb.innerHTML += '<br><small style="opacity:.7;cursor:pointer;" onclick="window.__mebu=true;this.parentNode.innerHTML+=\'<b> Cancelando…</b>\'">✕ Cancelar</small>';

  var r = await goToList();
  if (r.error) { _step('❌ Não abriu a lista: ' + r.error, '#c0392b'); return; }

  for (var i = 0; i < total; i++) {
    if (window.__mebu) { cancelado = true; break; }
    var d = collabs[i];
  ...
  if (cancelado) _step('⚠️ Cancelado. ' + found + '/' + total + ' QR codes enviados.', '#e67e22');
  else           _step('✅ ' + found + '/' + total + ' QR codes encontrados e enviados ao app!', '#00a650');
  window.__mebu = false;
})();
